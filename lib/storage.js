...
Database.prototype.put = function (rawDoc, buffer, opts, cb) {
  ...
  function check() {
    if (opts.primaryKey) doc._id = opts.primaryKey
    if (opts.overwrite || !doc._id) return store()
    
    debug('check', doc._id)
    self.get(doc._id, function(err, existing) {
      if (err) return store()
      if (opts.revise) return store(docUtils.updateRevision(existing, buffer, self.meta.json.columns))
      if (!doc._rev || doc._rev[0] < existing._rev[0]) return cb(self.errors.conflict())
      store()
    })
  }
  
  function store(opt_updated) {
    if (opt_updated) updated = opt_updated
    else if (!opts.overwrite) updated = docUtils.updateRevision(doc, buffer, self.meta.json.columns)
    else updated = doc
