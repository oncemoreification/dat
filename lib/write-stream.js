...
WriteStream.prototype.createWriteStream = function() {
  ...
  var writeStreams = []
  var shouldBatch = options.batch
  if (typeof shouldBatch === 'undefined') shouldBatch = true
  if (shouldBatch) {
    this.batchStream = byteStream(this.dat.dbOptions.writeBufferSize)
    writeStreams.push(this.batchStream)
  }
  this.writeStream = through(this.onWrite.bind(this), this.onEnd.bind(this))
  writeStreams.push(this.writeStream)
  
  pipeChain = pipeChain.concat(writeStreams)
...
WriteStream.prototype.writeBatch = function (rows) {
  ...
  for (var i = 0; i < len; i++) {
    ...
    var keys = docUtils.rowKeys(store.keys, store.sep, meta._id, meta._rev, seq, schema)
  ...
  function commit() {
    ...
    function next() {
      ...
      if (self.batchStream) self.batchStream.next()
